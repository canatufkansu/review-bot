...
- **Token counts for reviewers on a signed-in CLI.** A reviewer in **Signed-in CLI** mode is never priced — its cost is the subscription — and until now its tokens were not recorded either, so a panel of session reviewers showed no token figures at all. Claude reports its tokens in either mode, so they are now recorded beside the metered usage as a separate figure: the Statistics tab's **Tokens** tile counts everything the reviewers reported and says how much of it was on API keys and how much on subscriptions, the **Cost** tile stays metered-only, and a history entry shows "N tok · subscription". Nothing about billing changed: session tokens never carry a dollar figure and never appear in the usage table posted to GitHub. Codex and opencode report no tokens, so a panel of those two still shows none.
- **A running review shows how long it has been at it.** Each running item in the menu bar popover and on the Windows dashboard's queue carries a counter that ticks up from the moment the review started, so a review that is taking too long is visible as such rather than only as "Running".
- **A Statistics tab, and the average review time in the panel.** The settings window (and the Windows dashboard) now has a **Statistics** tab that reads the last 30 days of the activity history back as figures: how many decisions Review Bot posted and of which kind, how long a review takes from checkout to the posted decision (average, median and the most recent one), how long an author waited from the review request on GitHub to the decision, what share of the pull requests it requested changes on it later approved at a new commit and what share went on to merge, how many change requests it took before an approval followed, what share of its approvals have merged since, and the metered spend over the window. The menu bar popover shows the average review time, refreshed after every review, next to the window's approval and change-request counts. To make this possible every review's history entries now carry when the request was made on GitHub, when the review started and which commit it looked at, and a poll records — once per pull request — when a pull request Review Bot reviewed gets merged, which appears in the history as a **Merged** event. Entries written before this release are counted where they can be and left out of the averages.

- **Add as many models to the review panel as you like.** The panel was four fixed reviewers, one model each, because a reviewer's identity *was* its backend — so a second Claude model, or any provider Review Bot did not ship support for, was impossible without a new enum case, a new switch arm and a new CLI. A panel seat is now its own thing, and the Reviewers tab has a **More models** section where you add any model reachable over an OpenAI-compatible `chat/completions` endpoint: an aggregator such as OpenRouter, a provider's own compatibility endpoint, or a server on your own machine. Each card takes a name, an API base URL, a model name, a time limit, a key and per-million prices. Every added model joins the panel as a full reviewer — same review contract, same in-process agent loop, same read-only worktree sandbox DeepSeek uses — and takes part in the parallel run, strictest-verdict aggregation, the partial-panel disclosure, the decision policy, the failure budget, the injection checks and the usage table, named by whatever you called it. Adding one is the intended way to have several different models read the same pull request in detail.

  A few consequences worth knowing. Custom reviewers run after the built-ins, in the order you added them, so the posted review reads the same way on every run. A card that is still half-filled is skipped rather than run, so an incomplete row cannot put a failed reviewer in every review. Keys are filed under the reviewer's id rather than its name, so renaming one or repointing it at a different model never orphans its key — and removing the card removes the key with it. Like DeepSeek, a custom reviewer never adjudicates a reconciliation: it always bills a key, and giving the last word to an extra metered pass is the wrong default. And every model you add reads the whole pull request on its own key, so the panel's cost grows with it.

- **Choose which GitHub account Review Bot reviews as.** `gh` can be signed in to several accounts, and Review Bot always used whichever one was active — switching meant `gh auth switch`, which changes it for everything else on the machine too. The Repositories tab (and the Windows dashboard's Monitoring box) now offers **Review as**: `gh`'s active account, or any other account `gh` is signed in to. With an account chosen, every poll asks `gh` for that account's token and runs each `gh` and `git` of the poll with it — the search that finds review requests, the fetch, the post — so all three belong to the same account, while `gh`'s active account and git's global credential helper are left exactly as they were. Reviewer CLIs never see the token. An account `gh` is not signed in to fails the poll before any repository is touched, with a status that names it, rather than reviewing as whoever happens to be active. The status line now also says which account a poll ran as ("Watching 3 repositories as @alice").
- **Review Bot runs on Windows.** The engine — discovery, worktrees, the parallel reviewer panel, reconciliation, usage accounting, retries — is shared unchanged; what is new is a Windows shell around it. It lives in the notification area, and instead of a native settings window its icon opens a dashboard page in the browser with the same five tabs as the macOS settings window (Repositories, Reviewers, Decisions, Prompt, History), served by the app itself on `127.0.0.1` behind a per-launch token so nothing else on the machine can drive it. API keys go to the Windows Credential Manager, data lives under `%LOCALAPPDATA%\ReviewBot`, launch-at-sign-in is a per-user `Run` registry entry, and reviewer time limits are enforced with a kill-on-close job object so a cut-off CLI takes its child processes with it. Commands are resolved by Review Bot rather than a shell: an npm-installed CLI, which on Windows is a `.cmd` launcher, is unwrapped and run as `node.exe <script>` so the review prompt is never re-parsed by `cmd.exe`, and any other batch launcher is refused with a message saying why. The worktree sandbox now compares paths by component and case-insensitively on Windows, so a model cannot escape it by changing the case of a drive letter. `scripts\build-windows.ps1` packages a portable zip with the Swift runtime and, through Inno Setup (`Packaging\ReviewBot.iss`), a per-user `setup.exe` with a Start-menu entry, an uninstaller and an optional start-at-sign-in task that writes the same registry value the dashboard toggle does. CI builds and tests on Windows alongside macOS, and releases attach both packages next to the DMG.
- **Reviewer time limits are configurable.** Every reviewer was fixed at 15 minutes, and a large pull request that ran over produced nothing at all — on the pull request above, the CLI reviewer was cut off at exactly 900 seconds and the panel shrank to one. Each reviewer card now carries a **Time limit** stepper (1–240 minutes, default 15). It is per reviewer rather than global because they do not cost the same: a signed-in CLI on a flat subscription can be given an hour, while a reviewer on your own key is billed for the whole window, which the caption says next to the control. DeepSeek's internal budget and the engine's hard stop are both derived from the setting, so they cannot drift into the tie that would turn its graceful exit into a cancelled review.
- **DeepSeek as a fourth reviewer.** DeepSeek has no CLI, so unlike Claude, Codex, and opencode it is called directly over its chat-completions API. Review Bot runs the agent loop itself: the model is offered read-only `read_file`, `search`, and `list_files` tools confined to the review worktree, and each round of tool calls is executed locally and fed back until it produces its review. The diff, the PR thread, and the merge preview are also inlined in the opening message, so a model that rejects tools (DeepSeek's reasoning models do) still falls back to a complete single-shot review. DeepSeek takes part in the parallel run, strictest-verdict aggregation, the partial-panel disclosure, the decision policy, the failure budget, and the pre-approval injection checks exactly like the CLI reviewers. It is deliberately last in the reviewer order, so it is never the one that adjudicates a reconciliation — the step with final say over the decision stays with a reviewer that inspects the worktree under its own CLI sandbox, and never costs an extra metered pass.

  The review is asked for explicitly rather than inferred. DeepSeek has no separate reasoning channel — verified against the API, its responses carry no `reasoning_content` field — so its chain-of-thought and its final answer both arrive in `content`, and a turn without tool calls can equally mean "I'm finished" or "I paused to think". Treating the latter as the review posted the model's monologue to GitHub. Now, once the model stops calling tools (or exhausts its rounds, or its wall-clock budget), Review Bot only accepts that turn if it already carries the contract's structure; otherwise it sends one more request that restates the required sections and offers no tools, and uses that reply. The narration stays in context as history the model can draw on, but never reaches the review. Any remaining preamble is trimmed to the contract's first heading, matching the variants models actually emit (`## Summary:`, `### Summary`, `## **Summary**`, and so on); output with no such heading is posted unchanged rather than risk discarding a review.

  Its two money-losing failure paths are handled the same way the CLI reviewers' are. A DeepSeek request that times out is recorded as a timeout, so the in-review retry does not answer a hung agent loop with a second full paid one; and a loop that spends a dozen billed rounds before failing carries that spend out with the failure, so a review that cost real money is never recorded as free.

- **Per-reviewer sign-in mode, with API keys in the login Keychain.** Each reviewer now chooses between **Signed-in CLI** (unchanged behavior — Review Bot sends no credentials and the CLI uses its own login) and **API key**. In key mode Claude and Codex are run with `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` set from the key you saved; in session mode that variable is explicitly unset, so a key exported in your shell can no longer be picked up by accident. DeepSeek is API-key-only, since there is no CLI session for it to fall back on. opencode has no key path at all — it authenticates through its own configuration — so its card offers no credential picker. Keys are stored in the macOS Keychain and never written to `config.json`; a reviewer set to key auth with no saved key fails with an explanatory message instead of silently falling back to a different account, and that failure is classified as terminal, so the reviewer is not run a second time inside the same review on a condition only a settings change can fix. Keys are read once per review, before the reviewers start, and never on the thread that draws the window or the one that runs the reviewers — a Keychain prompt blocks whatever thread raises it until you answer, so it must not be able to freeze Settings or hold up the reviewers running in parallel. That also means a rebuilt app asks at most once per reviewer per review.
- **API keys can be supplied through the environment.** For a reviewer already set to **API key**, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, and `DEEPSEEK_API_KEY` are now read from Review Bot's own environment when set, taking precedence over the Keychain, so a development run or a one-off script can supply a key without saving one. This does not change the default **Signed-in CLI** mode, which still unsets those variables — exporting a key without switching the mode leaves that reviewer on its CLI's own login. A GUI app launched from Finder or at login inherits launchd's environment rather than a shell's, so this is a development affordance — the packaged app still reads the Keychain.
- **Token usage and cost per review, for the reviewers you pay per token.** Now that a reviewer can be billed to a key of your own, each review records what it consumed. Every provider call is counted — each round of DeepSeek's agent loop, an attempt that failed and the extra attempt when a reviewer is re-run inside the same review, both of which were billed all the same, and the reconciliation pass, which is charged to the reviewer that adjudicated — and the total is written to the activity history for each review, so spend stays traceable per pull request and can be totalled from `history.json`. A **Reviewers → Usage and cost** toggle controls whether the same figures are also appended to the posted GitHub review; tracking continues either way. Claude is now run with `--output-format json` so its own token counts and dollar figure come back with the review, and its output is parsed defensively: a `claude` that accepts the flag but prints something other than that envelope is still read as the review itself. DeepSeek's API reports the tokens a review consumed but never a price, so its cost is computed from per-million rates — input, cached input, and output — that you keep on its reviewer card: published prices drift, and a stale built-in number would report the wrong spend without ever saying so. Rates are written with a dot or a comma, since a comma-decimal locale would otherwise read a pasted `0.27` as `27`; setting all three to zero reports tokens and no cost. A `config.json` written before prices existed is given the defaults when it loads, and the posted usage table discloses that DeepSeek's figures come from these settings rather than from the provider. Codex reports no usage, and its card says so when it is set to API-key auth — the mode in which it would be metered. Reviewers using a signed-in CLI are deliberately left out, because their cost is a flat subscription and attributing dollars to a single review would be fiction; that covers opencode in every configuration, since it has no API-key mode at all. A cost that cannot be determined is shown as unknown rather than as `$0.00` — including a DeepSeek review whose responses carried no token counts, because a rate multiplied by unknown tokens is unknown and not free.

### Fixed

- **A reviewer that has hit its usage limit no longer holds the review for its whole time limit.** opencode, told by its provider that the usage limit is exceeded, logs the error and then never exits — on a panel with a 15-minute limit that was fifteen minutes per review, on every review, with the posted panel saying only "timed out". Two things changed. Every CLI reviewer now runs under an output watch: the moment a line of what it writes reads as a terminal failure — an exhausted quota, a rejected login, an unusable model, the same classification that already decides whether a reviewer is retried — the run is stopped and the panel goes on without it within seconds, and that failure is not retried inside the review. And opencode is run with `--print-logs --log-level ERROR`, because without that its error never reaches stdout or stderr at all; its log record is reduced to the quoted error ("AI_APICallError: Go usage limit exceeded") in the posted disclosure. A reviewer that does time out now carries the last line it wrote into the failure, so a hang that announced itself is explained rather than reported as a bare timeout.

### Changed

- **Review Bot keeps checking GitHub while it reviews.** A poll used to list the review requests and then review every one of them before returning, and no further poll ran until it had — so a request that arrived a minute into a long queue waited for the whole queue *and* the next poll interval before Review Bot even noticed it. Discovery and reviewing are now separate: a poll only lists and queues, the reviews run on behind it (still `maxConcurrentReviews` at a time), and the next poll runs on schedule while they do, queueing whatever is new behind them. **Run now** likewise checks GitHub at any time and is only unavailable while a check is already in progress; a request that is already queued or under review is not announced or queued twice. Each queued request is reviewed with the settings it was found under, so changing the panel mid-queue never switches a review halfway through.
- **The chat-completions reviewer is no longer DeepSeek-specific.** `DeepSeekClient` and `DeepSeekReviewer` are now `ChatCompletionsClient` and `ChatCompletionsReviewer`, and DeepSeek is one endpoint they are pointed at rather than the thing they are. Provider failures are reported provider-neutrally ("The provider returned HTTP 401…"), with the reviewer's own name already on the failure, and a rejected key or an empty balance is classified as terminal for every chat-completions reviewer rather than only for DeepSeek — so a custom reviewer with a bad key fails once instead of spending the in-review retry and then the review's failure budget.

- Reviewers are now discovered from a single list rather than a hardcoded roster, so every enabled reviewer runs in parallel, appears in a stable order in the posted review, and is dispatched from one place that a newly added reviewer cannot silently fall out of. The reconciliation adjudicator is chosen from the reviewers that are actually enabled, in the same order — Claude, then Codex, then opencode — rather than falling through a fixed preference chain that could spawn a reviewer the panel does not include.
- The failure-budget captions on the Reviewers tab now say that every retried attempt re-runs the reviewers, so a reviewer billed to your own API key is charged again.
...
- **A reviewer that says it could not assess a pull request no longer produces an approval.** The review contract requires a trailing verdict line, so a reviewer that spent its whole turn failing to reach the diff still signed off with `NITS_ONLY` — "I found no problems" being literally true of a review that looked at nothing — and the panel read that as an approval. This happened on a 181-file pull request. The body now overrides the verdict line: a review that states in its own words that it could not be assessed has its verdict withdrawn and does not count toward the decision, and it is not re-run inside the same review, because a second pass re-reads the same unreadable evidence and, for a reviewer billed to your own key, costs a second full bill for the same non-answer. If another reviewer did finish, the decision is theirs and the withdrawal is disclosed in the posted body. If none did, Review Bot now posts a neutral comment carrying each reviewer's own account of why, rather than staying silent and retrying until the failure budget gives up without ever telling the author anything.

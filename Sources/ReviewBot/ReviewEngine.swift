...
actor ReviewEngine {
    ...
    private func runReconciliation(
        ...
        // whose panel run failed is unlikely to answer this call either, and an adjudicator that
        // fails falls back to the strictest verdict, which is the lone blocker reconciliation
        // exists to check. The case that made this concrete: Claude out of quota, Codex
        // `SHOULD_FIX`, opencode `CLEAN` — the disagreement reconciles through Claude, Claude
        // fails again, and the blocker gates the pull request unexamined. Codex adjudicates it
        // instead. Preferring a survivor never weakens the check: reconciliation only runs when
        // two reviewers parsed verdicts, so it always has a candidate that finished, and the
        // fallback below keeps the old configuration-only order for any caller where none did.
